...
def calculate_base_difficulty(level: int, overall_streak: int) -> int:
    """Calculate base difficulty for quest generation (scales with level limitlessly)."""
    # Cấp độ khó sẽ tăng tuyến tính với cấp độ người chơi
    base = 1 + level // 3  # Increase difficulty every 3 levels
    streak_boost = min(overall_streak // 7, 3)
    return min(base + streak_boost, 50)  # Nâng cap tối đa lên 50 thay vì 10
...
def scale_quest_requirements(base_value: int, difficulty: int, level: int) -> int:
    ...
    scale_factor = 1 + (difficulty - 1) * 0.15 + level * 0.05
