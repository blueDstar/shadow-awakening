...
Updated to scale rewards by level and breakthrough phase.
"""
import json
import uuid
import random
...
from app.models import (
    ...
    DailyQuest, QuestHistory, Streak, Quest
)
...
def get_phase_value(stat_cap: Optional[StatCap]) -> int:
    """Normalize phase so legacy rows never break reward scaling."""
    if not stat_cap:
        return 0
    try:
        return max(int(stat_cap.phase or 0), 0)
    except Exception:
        return 0


def get_reward_scaling(level: int, phase: int) -> Dict[str, float]:
    """Global reward multipliers used by quests/rewards/breakthrough."""
    safe_level = max(int(level or 1), 1)
    safe_phase = max(int(phase or 0), 0)
    return {
        "exp_multiplier": round(1 + safe_phase * 0.15 + safe_level * 0.02, 3),
        "stat_multiplier": round(1 + safe_phase * 0.10 + safe_level * 0.015, 3),
    }
...
async def generate_daily_quests(db: AsyncSession, user: User, target_date: Optional[date] = None) -> List[DailyQuest]:
    ...
    if fitness_templates:
        ...
        for template in selected_fitness:
            quests.append(_build_quest_from_template(template, difficulty, level, "main"))
...
async def refresh_daily_quests(db: AsyncSession, user: User, target_date: Optional[date] = None) -> List[DailyQuest]:
    ...
    alt_templates = templates_pool.get(alt_cat, templates_pool.get("fitness", []))
    if alt_templates:
        new_quest_data.append(_build_quest_from_template(random.choice(alt_templates), difficulty, level, "side"))
    ...
    for q in new_quest_data:
        exp = int(calculate_quest_exp(difficulty, q["quest_type"], level, q.get("base_exp"), phase) * 1.2)
...
def _generate_main_quests(
    ...
    for stat_name in weak_stats[:count]:
        ...
        quests.append(_build_quest_from_template(random.choice(templates), difficulty, level, "main"))
...
def _generate_side_quests(
    ...
    for cat in cats[:count + 2]:
        if len(quests) >= count:
            break
        templates = templates_pool.get(cat, [])
        if templates:
            quests.append(_build_quest_from_template(random.choice(templates), difficulty, level, "side"))
...
def _generate_habit_quests(
    ...
    return [_build_quest_from_template(template, difficulty, level, "habit") for template in habit_mix[:count]]
...
def _generate_penalty_quests(
    ...
    template = random.choice(matching or PENALTY_TEMPLATES)
    return [_build_quest_from_template(template, max(1, difficulty - 1), level, "penalty")]
...
def _build_quest_from_template(
    ...
    return {
        ...
        "title_vi": template.get("title_vi", "Nhiệm vụ ẩn"),
        "title_en": template.get("title_en", "Hidden Quest"),
        "desc_vi": template.get("desc_vi", ""),
        "desc_en": template.get("desc_en", ""),
    }
...
def _apply_chain_continuity(
    ...
    completed_categories = {yq.category for yq in yesterday_quests if yq.status == "completed"}
    current_categories = {q["category"] for q in quests}

    for cat in completed_categories:
        if cat not in current_categories and cat in templates_pool and templates_pool[cat]:
            quests.append(_build_quest_from_template(random.choice(templates_pool[cat]), difficulty, level, "side"))
            break
...
def _find_weak_stats(stats: dict, cap: int) -> List[str]:
    threshold = cap * 0.4
    weak = [stat_name for stat_name in CORE_STATS if stats.get(stat_name, 0) < threshold]
    random.shuffle(weak)
...
async def _get_recent_performance(db: AsyncSession, user_id) -> Tuple[int, int]:
...
async def reroll_daily_quest(db: AsyncSession, user: User, quest_id: uuid.UUID) -> Optional[DailyQuest]:
    result = await db.execute(
        select(DailyQuest).where(
    ...
    if not old_quest or old_quest.is_rerolled:
        return None
    ...
    other_categories = [c for c in allowed_categories if c != old_quest.category] or (allowed_categories or all_categories)
    ...
    scaled_stat_rewards = {}
    for stat_name, val in q.get("stat_rewards", {}).items():
        scaled_gain = calculate_stat_gain(float(val), old_quest.difficulty, overall_streak, level, phase)
        scaled_stat_rewards[stat_name] = max(1, int(round(scaled_gain)))
    ...
    old_quest.exp_reward = calculate_quest_exp(old_quest.difficulty, old_quest.quest_type, level, q.get("base_exp"), phase)
